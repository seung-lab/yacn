...
We show that the error-detecting net can provide ``advice'' to the  error-correcting net in the following way. To create the candidate object mask for the error-correcting net from a baseline segmentation, one can simply take the union of all erroneous segments as found by the error-detecting net. Since the typical error rate in the baseline segmentation is already low, there is often not much for the error-correcting net to erase and the task is relatively easy.  The idea of using the error detector to choose locations for the error corrector was proposed previously though not actually implemented \cite{multipass}. Furthermore, the idea of using the error detector to not only choose locations but provide ``advice'' is novel as far as we know.

%We find object mask pruning to be a very natural task. For an object containing a few merge errors, the correct pruning of the mask can often be determined without consulting the original image at all.  For object mask extension, on the other hand, information in the image is required to correctly extend the mask.  For the mask pruning task, one could argue that the mask is the main input and the image is an auxiliary input.  For the mask extension task, the mask serves to focus attention on a particular location or object in the image, which is the main input.
