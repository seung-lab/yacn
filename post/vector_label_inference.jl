...
otpt = zeros(Float32, (size(inpt)...,8))
forward!(inpt,otpt,patch_size,x->(permutedims(adversarial.main_model[:compute_vector_labels](permutedims(x,[3,2,1])),[3,2,1,4])), step=0.99)

h5open("$(dirname(filename))/$(set)_vec_$(filename_prefix).h5", "w") do file
